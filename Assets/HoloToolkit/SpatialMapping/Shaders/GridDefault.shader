...
Shader "Custom/GridDefault"
{
	Properties
	{
		_LineScale("LineScale", Float) = 0.02
		_LinesPerMeter("LinesPerMeter", Float) = 2
	}
			...
			Pass
		{
			...
			fixed4 frag(VertToFrag i) : SV_Target
			{
				...
				if (abs(normal.y) < 0.3f && abs(wpmod.y) < _LineScale * _LinesPerMeter)
				{
					ret = float4(1, 1, 1, 1);
				}
				if (abs(normal.x) > 0.3f && abs(wpmod.x) <  _LineScale * _LinesPerMeter)
				{
					ret = float4(1, 1, 1, 1);
				}
